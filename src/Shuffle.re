...
type transition = {
  ...
  westStartYInv: float,
  northStartYInv: float,
  eastStartXInv: float,
  southStartYInv: float,
  westStartXInv: float,
  northEndX: float,
  eastEndY: float,
  southEndX: float,
...
let impureGetTimeBasedSeedUpTo60k = () => {
  ...
  int_of_float(
    Js.Date.getSeconds(now) *. 1000.0 +. Js.Date.getMilliseconds(now),
  );
};
...
let getShuffledPack = () => {
  // IMPURE!
  let () =
    shuffleArrayInPlace(myArrayOfIndices, impureGetTimeBasedSeedUpTo60k());
  ...
  let shuffledPack =
    Array.map(
      card =>
        {
          ...card,
          shuffleIndex: myArrayOfIndices[card.noTrumpValue],
          lifecycle: Hand,
        },
      initialPack,
    );
