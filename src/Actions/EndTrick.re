// common code for ending trick(s)
let getNextStateFromTricksWonAndWinningPartnership =
    (
      state: TopLevel.state,
      qtyTricksToGiveWinningPartnership: int,
      winningDiscardPoc: string,
    ) => {
  let totalTricksNorthSouthIncrement =
    winningDiscardPoc === "North" || winningDiscardPoc === "South"
      ? qtyTricksToGiveWinningPartnership : 0;
  let totalTricksWestEastIncrement =
    winningDiscardPoc === "West" || winningDiscardPoc === "East"
      ? qtyTricksToGiveWinningPartnership : 0;
  Js.log("totalTricksNorthSouthIncrement:");
  Js.log(totalTricksNorthSouthIncrement);
  Js.log("totalTricksWestEastIncrement:");
  Js.log(totalTricksWestEastIncrement);
  // need this too for legacy reason
  ...
  };
  {
    ...
    chicagoScoreSheet:
      ...
      && (
        scoreSheetRecord.scoreNorthSouth == None
        || scoreSheetRecord.scoreWestEast == None
      )
        ? [
...
let execute = (state: TopLevel.state) => {
  ...
  //   winningDiscardPoc === "North" || winningDiscardPoc === "South" ? "NS" : "EW";
  // we need to pass in the winningDiscardPoc, not just partnership, as this is used in the returned state

  // at this point, both the above code and the Concede code are in a position to pass common paremeters
  // and execute identical finctionality to end the trick(s)

  getNextStateFromTricksWonAndWinningPartnership(state, 1, winningDiscardPoc);
  // let totalTricksNorthSouthIncrement =
